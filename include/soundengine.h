...
namespace imajuscule::audio {
  ...
  enum class FreqXfade : unsigned char {
    ...
    public:
      ...
      bool orchestrate(Chans & chans, int max_frame_compute) {
        if(ramp_specs.done()) {
          return false;
        }
        ...
        return playNextSpec(chans);
      }
      
      template<typename Chans>
      bool playNextSpec(Chans & chans) {
        while(true) {
      ...
      initialize_sweep(Chans & chans,
        ...
        return do_initialize(chans, initialize, 0, 0, 1, 0, articulative_pause_length, pan);
      }
      
      template<typename Chans>
      std::pair<std::function<void(void)>,std::function<bool(Chans&,int)>>
      initialize_birds(Chans & chans,
        ...
        return do_initialize(chans, initialize, start_node, pre_tries, min_path_length, additional_tries, articulative_pause_length, pan);
      }
      
      template<typename Chans>
      std::pair<std::function<void(void)>,std::function<bool(Chans&,int)>>
      initialize_wind(Chans & chans,
                      int start_node, int pre_tries, int min_path_length, int additional_tries, SoundEngineInitPolicy init_policy,
                      float pan) {
        bool initialize = (!markov) || (init_policy==SoundEngineInitPolicy::StartAfresh);
        if(!markov) {
                    ...
                    float pan)
      {
        ...
        return {
          {},
          [this](Chans & chans, int max_frame_compute){
            auto i = goOn();
            if(0==i) {
              return false;
            }
            
            auto res = orchestrate(chans, max_frame_compute);
            if(!res) {
              tryIncrementOddOn(i);
            }
            return res;
